...
class Gallery extends CI_Controller {
	...
	public function addImg(){
        ...
        if($this->input->post('submit') && !$data['user']['disabled']) {
			...
			$price    	= $this->input->post('image-price');
			...
			if($_FILES['gImage']['tmp_name'] == ''){
				...
				if($validation && $this->upload->do_upload('gImage')) {
					...
					$data['alert'] = array('type' => 'alert alert-success', 'msg' => 'Product Added Successfully.');
					redirect(base_url(GALLERY_CONTROLLER . '/listGallery'));
				}
			}
        }
        ...
        if($gallery = $this->GalleryModel->getGallery($id)) {
            ...
            if($this->input->post('submit') && !$data['user']['disabled']) {
                ...
                if($validation) {
                    ...
                    $this->session->set_flashdata('alert', array('type' => 'alert alert-success', 'msg'  => 'Successfully updated product.'));
	...
	public function deleteImg($id = null, $confirm = false) {
		...
		if($confirm && !$data['user']['disabled']) {
			...
			$this->session->set_flashdata('alert', array('type' => 'alert alert-success', 'msg'  => 'Successfully delete Product.'));
		}
